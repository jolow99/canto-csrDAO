...
contract csrDAOTreasury is Ownable, IERC721Receiver {
    ...
    /// @dev Mapping from donor address to the token IDs staked
    mapping(address => uint[]) public stakedTokenIds; 
...
/// @notice returns the entire array of stakedTokenIds
    /// @dev this is needed because the default getter generated by compiler only allows you to access the values in the array
    function getStakedTokenIds(address donor) external view returns (uint[] memory) {
        return stakedTokenIds[donor];
    }
    ...
    function withdrawCsrNft(uint tokenId, address withdrawTo) external onlyNftDonor(tokenId) {
        // Remove tokenId from the array of tokenIds in stakedTokenIds
        uint[] storage tokenIds = stakedTokenIds[msg.sender];
